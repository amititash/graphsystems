  ...
  public boolean addElements(String line)
  {
    ...
    Vertex order = 
      g.V().has("order_id",values[1]).fold().
            coalesce(__.unfold(),__.addV().property("order_id",values[1]).property("label", "order")
            .property("order_date", values[2]).property("ship_date", values[3]).property("ship_mode", values[4])).next();

    
    
    Vertex product = 
      g.V().has("product_id",values[13]).fold().
            coalesce(__.unfold(),__.addV().property("product_id",values[13]).property("label", "product")
            .property("product_name", values[16])).next();
    
      Vertex customer = 
      g.V().has("customer_id",values[5]).fold().
            coalesce(__.unfold(),__.addV().property("customer_id",values[5]).property("label", "customer")
            .property("customer_name", values[6])).next();
    
    Vertex category = 
      g.V().has("category_name",values[14]).fold().
            coalesce(__.unfold(),__.addV().property("category_name",values[14]).property("label", "category")).next();
    
    Vertex sub_category = 
      g.V().has("sub_category_name",values[15]).fold().
            coalesce(__.unfold(),__.addV().property("sub_category_name",values[15]).property("label", "sub_category")).next();

    Vertex region = 
      g.V().has("region_name",values[12]).fold().
            coalesce(__.unfold(),__.addV().property("region_name",values[12]).property("label", "region")).next();
    
    Vertex segment = 
      g.V().has("segment_name",values[7]).fold().
            coalesce(__.unfold(),__.addV().property("segment_name",values[7]).property("label", "segment")).next();

     Vertex postal_code = 
      g.V().has("postal_code",values[11]).fold().
            coalesce(__.unfold(),__.addV().property("postal_code",values[11]).property("label", "postal_code")).next();

     Vertex city = 
      g.V().has("city_name",values[9]).fold().
            coalesce(__.unfold(),__.addV().property("city_name",values[9]).property("label", "city")).next();
    
    Vertex state = 
      g.V().has("state_name",values[10]).fold().
            coalesce(__.unfold(),__.addV().property("state_name",values[10]).property("label", "state")).next();
    
    Vertex country = 
      g.V().has("country_name",values[8]).fold().
            coalesce(__.unfold(),__.addV().property("country_name",values[8]).property("label", "country")).next();
 ...
 */
   Vertex sales = tg.addVertex("label", "sales", "sales_amount", values[17]);
    Vertex quantity = g.addV().property("label", "quantity", "quantity_value", values[18]); // RELATIONSHIP NOT ADDED
    Vertex discount = tg.addVertex("label", "discount", "discount_amount", values[19]);
    Vertex profit = tg.addVertex("label", "profit", "profit_amount", values[20]);
...
// Now create all the edges defining properties and relatinships
  ...
  public void displayGraph()
  {
    ...
    try
    {
      ...
      tg.io(IoCore.graphml()).writeGraph("minigraph.graphml");
  ...
  public static void main(String[] args) 
  {
    ...
    if (gcsv.createGraph())
    {
      try 
      {
        ...
        FileReader fileReader = new FileReader("minigraph.csv");
